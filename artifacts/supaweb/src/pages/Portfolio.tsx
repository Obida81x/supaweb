...
const categories = ["All", "Business Website", "E-commerce", "Dashboard", "Full Stack App"];
      ...
      <section className="py-24 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass text-xs font-medium text-violet-300 mb-6">
            <Zap className="w-3 h-3" /> Our Work
          </motion.div>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-zinc-400 text-lg">
            A showcase of projects we have built for clients across industries.
          </motion.p>
        </div>
      </section>
                ...
                <div key={i} className="glass rounded-2xl overflow-hidden animate-pulse">
                  ...
                  <div className="p-6 space-y-3">
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3].map((j) => <div key={j} className="h-6 w-16 bg-white/5 rounded-md" />)}
                    </div>
                  </div>
                </div>
              ))}
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {project.technologies.map((tech) => (
                          <span key={tech} className="px-2.5 py-1 rounded-lg bg-violet-500/8 text-violet-300 text-xs font-medium border border-violet-500/15">
                            {tech}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                        ...
                        {!project.liveUrl && !project.githubUrl && (
                          <span className="text-xs text-zinc-600">Private project</span>
                        )}
                      </div>
                    </div>
                  </motion.div>
